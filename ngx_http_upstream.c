    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    c = r->cache;
    pe = c->file.name.data + c->file.name.len;
    pe--; // skip trailing '\0'

    v->valid = 1;
    v->no_cacheable = 0;
    v->not_found = 0;
    // do not make c->key dump
    // take the key dump from the file name
    for (p = pe; (p > c->file.name.data) && (*p != '/'); p--)
    {
      v->data = p; // exit with data[0] or without leading path separator
    }
    v->len = (pe > p) ? pe - p : 0; // protect against empty file name (if at all possible)
